...
learners, and the out-of-bag (OOB) samples used for testing each tree, automatically
...
(RF) [Breimann, 2001](link) on metabolomics data. This has been relatively successful,
with decent sensitivity and specificity, and hopefully I'll be able to post more
soon. However, PLS [Wold, 1975](link) is a standard technique used in metabolomics
...
Just to show that the data is pretty random, lets use PCA to do a decomposition
...
The confusion matrix comparing actual *vs* predicted classes based on the OOB
samples:
...
And an overall error of `r rf_model$err.rate`.
...
And voila! Perfectly separating data! If I didn't tell you that it was random, would
you suspect it?

## Cross-validated PLS-DA

Of course, one way to truly assess the worth of the model would be to use cross-validation,
where a fraction of data is held back, and the model trained on the rest.